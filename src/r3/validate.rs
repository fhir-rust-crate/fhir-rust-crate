...
//!
//! R3 has no `canonical` or `url` primitive — both arrived in R4 — so this
//! module checks two fewer types than its R4 and R5 counterparts.
//!
//! Provides format checks for the FHIR R3 primitive datatypes (the
//! regular-expression constraints from the specification, implemented without
//! an external regex dependency), and bridges validation results into an R3
//! `OperationOutcome`.
//!
//! The [`Validate`] trait and [`ValidationIssue`] type are shared by every FHIR
//! release and live in [`crate::validate`]; this module re-exports them so R3
//! code can use `fhir::r3::validate::Validate`. Recursive validation of complex
//! types and resources is generated by `#[derive(Validate)]`.
//!
//! # Examples
//!
//! ```
//! use fhir::r3::types::Id;
...
mod tests {
    ...
    fn invariant_ext_1() {
        ...
        // R3 types `Extension.url` as a `uri`; R4 and R5 use a `string`.
