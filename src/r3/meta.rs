//! Element metadata extracted from the FHIR R3 specification.
//!
...
//! it — regenerate with `cargo run -- r3`.
//!
...
/// Look up the metadata for an R3 element by its full FHIR path.
///
...
/// Every extracted R3 element, sorted by path.
#[must_use]
...
mod tests {
    ...
    // `Observation.value[x]` allows 11 types in R3 and 11 in R4 — but not the
    // same 11. R3 permits `Attachment` and not `integer`; R4 reversed both, and
    // R5 then allows all of them plus `Reference`. This is exactly the kind of
    // difference the separate models exist to keep apart.
    #[test]
    fn observation_value_choice_types() {
        let value = element("Observation.value[x]").expect("Observation.value[x]");
        assert!(value.is_choice());
        assert_eq!(value.types.len(), 11, "R3 Observation.value[x] type count");
        for code in ["Quantity", "CodeableConcept", "string", "Range", "Period", "Attachment"] {
            assert!(value.type_codes().any(|c| c == code), "missing type {code}");
        }
        assert!(!value.type_codes().any(|c| c == "integer"), "integer arrived in R4");
        assert!(!value.type_codes().any(|c| c == "Reference"), "Reference arrived in R5");
    }
