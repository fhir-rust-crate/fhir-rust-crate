//! Utilities for working with R3 `Bundle`s: iterating the contained resources,
//! following `next`-link paging, and building transaction/batch bundles.
//!
//! ```
//! use fhir::r3::resources::{Bundle, Patient};
//!
...
use crate::r3::codes::{BundleType, HttpVerb};
use crate::r3::resources::bundle::{Bundle, BundleEntry, BundleEntryRequest};
use crate::r3::resources::Resource;
use crate::r3::types;
...
impl Bundle {
    ...
    ///
    /// R3 types `Bundle.link.relation` as a plain `string`; R5 later bound it
...
mod tests {
    ...
    fn transaction_builder_produces_entries() {
        let patient = Patient { id: Some(types::Id("p1".to_string())), ..Default::default() };
        let bundle =
