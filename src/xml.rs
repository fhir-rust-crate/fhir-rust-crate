    ...
    loop {
        match reader.read_event()? {
            Event::Start(e) => {
                ...
                read_children(table, &mut reader, &name, &name, &mut obj)?;
...
fn read_children(
    ...
    loop {
        match reader.read_event()? {
            Event::Start(e) => {
                let name = String::from_utf8_lossy(e.name().as_ref()).into_owned();
                let child = read_element(table, reader, &name, context, &e, false)?;
                insert_child(table, obj, &name, context, child);
            }
            Event::Empty(e) => {
                let name = String::from_utf8_lossy(e.name().as_ref()).into_owned();
                let child = read_element(table, reader, &name, context, &e, true)?;
                insert_child(table, obj, &name, context, child);
            }
            Event::End(e) => {
...
fn read_element(
    ...
    read_children(table, reader, name, &child_context, &mut obj)?;
    Ok(Value::Object(obj))
}
...
fn element_meta(
    ...
    if let Some(e) = crate::meta::find(table, path) {
        return Some(e);
    }
