...
**Two releases are modelled**, each complete and independent:
...
**The releases are separate types on purpose.** An R4 `Patient` is not an R5
`Patient`; the releases disagree about which elements exist and what they mean,
...
- **`src/r4/` is entirely generated** by `cargo run -- r4`. Do not hand-edit
  it; change the generator and regenerate.
...
| --- | --- |
| Build | `cargo build` |
| Build every release | `cargo build --all-targets --features r4` |
| Test (unit + doctests) | `cargo test` |
| Test every release | `cargo test --features r4` |
| Doctests only | `cargo test --doc` |
| Lint (pedantic; must be 0) | `cargo clippy --all-targets -- -D warnings` |
| Lint every release | `cargo clippy --all-targets --features "r4 xml client" -- -D warnings` |
| Docs (deny warnings) | `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps --features "r4 xml client"` |
| Feature builds | `cargo test --features client` / `--features xml` / `cargo build --features precise-decimal` |
| Regenerate R4 | `cargo run -- r4` (rewrites `src/r4/`) |
...
**R4 is off by default**, so anything exercising it needs `--features r4`.
...
This crate is currently 100% green. With `--features "r4 xml client"` that is
1100 unit tests + 876 doctests and 0 clippy warnings. Do not regress it. If you
touch the model, re-run the full gate **for every release you touched** — `r4`
is off by default, so a plain `cargo test` will not notice an R4 regression.
CI additionally enforces `doc -D warnings`, the MSRV (1.88), the feature builds
(including R4-only, `--no-default-features --features r4`), the mdBook build,
and the proc-macro publish dry-run.
